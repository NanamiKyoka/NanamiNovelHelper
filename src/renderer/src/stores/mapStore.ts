...
function applyDiff(base: MapData, diff: MapDataDiff): MapData {
  return {
    ...
    backgroundColor:
      diff.backgroundColor !== undefined ? diff.backgroundColor : base.backgroundColor,
...
export const useMapStore = create<MapState>((set, get) => ({
  ...
  moveChunkToHex: (chunkId: string, hexPosition: HexPoint) => {
    ...
    const existingChunk = currentMap.data.chunks.find(
      c =>
        c.id !== chunkId && c.hexPosition.q === hexPosition.q && c.hexPosition.r === hexPosition.r
    )
  ...
  deleteElement: (elementId: string) => {
    ...
    const newChunks = currentMap.data.chunks.map(chunk => {
      ...
      if (
        updatedChildren.length !== chunk.children.length ||
  ...
  addConnection: (options: CreateConnectionOptions) => {
    ...
    const exists = currentMap.data.connections.some(
      c =>
        (c.sourceChunkId === options.sourceChunkId && c.targetChunkId === options.targetChunkId) ||
  ...
  connectChunks: (sourceChunkId: string, targetChunkId: string) => {
    ...
    const exists = currentMap.data.connections.some(
      c =>
        (c.sourceChunkId === sourceChunkId && c.targetChunkId === targetChunkId) ||
