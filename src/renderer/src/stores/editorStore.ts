...
interface EditorState {
  ...
  goToPositionRequest: { filePath: string; matchText: string; line: number; column: number } | null
  requestGoToPosition: (filePath: string, matchText: string, line: number, column: number) => void
...
export const useEditorStore = create<EditorState>()(
  persist(
    (set, get) => ({
      ...
      // 请求跳转到指定匹配文本（包含文件路径、行号、列号）
      requestGoToPosition: (filePath: string, matchText: string, line: number, column: number) => {
        set({ goToPositionRequest: { filePath, matchText, line, column } })
      },
