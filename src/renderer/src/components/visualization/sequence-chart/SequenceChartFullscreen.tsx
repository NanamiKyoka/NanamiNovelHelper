...
function SequenceChartFullscreen({ chartId, onBack }: SequenceChartFullscreenProps): JSX.Element {
  ...
  const [newEventColor, setNewEventColor] = useState(DEFAULT_COLORS.event)
  ...
  const resetAddForm = () => {
    ...
    setNewEventColor(DEFAULT_COLORS.event)
  }
  ...
  const handleProgressDragStart = useCallback(
    (e: React.MouseEvent, event: SequenceEvent) => {
      e.preventDefault()
  ...
  const handleLabelSave = useCallback(async () => {
    ...
    if (editingLabelText.trim() === '') {
      ...
      if (idx >= 0) {
        ...
        newLabels = [
          ...existingLabels,
          { position: editingLabelPos, label: editingLabelText.trim() }
        ]
      }
    }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.toolbarRight}>
          ...
          <Input
            ...
            style={{ width: 160 }}
          />
          <Button icon={<ExpandOutlined />} onClick={() => setExpandModalVisible(true)}>
            扩展单元格
          </Button>
            ...
            {CHART_PALETTE.map(c => (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border:
                    newEventColor === c ? '2px solid var(--text-primary)' : '2px solid transparent',
        ...
        {editingEvent && (
          <>
            ...
            <div className={styles.formItem}>
              <label className={styles.formLabel}>进度</label>
              <Slider
                value={editingEvent.progress}
                onChange={v => setEditingEvent({ ...editingEvent, progress: v })}
                min={0}
                max={100}
              />
            </div>
            ...
            <div className={styles.formItem}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <ColorPicker
                  value={editingEvent.color || DEFAULT_COLORS.event}
                  onChange={color =>
                    setEditingEvent({ ...editingEvent, color: color.toHexString() })
                  }
                  showText
                />
              </div>
      ...
      >
        <div className={styles.formItem}>
          ...
          <InputNumber
            min={10}
            max={500}
            value={expandCellCount}
            onChange={v => setExpandCellCount(v || 100)}
            style={{ width: '100%' }}
          />
      ...
      <Dropdown
        menu={{
          items:
            contextMenu.type === 'event' && contextMenu.targetId
