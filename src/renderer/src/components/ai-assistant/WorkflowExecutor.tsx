...
interface SkillVariableInputProps {
  ...
  onExecuteSkill: (skillId: string, params: Record<string, unknown>) => Promise<SkillExecutionResult>
}

function SkillVariableInput({ variable, onChange, skills, onExecuteSkill }: SkillVariableInputProps): JSX.Element {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (execResult.success) {
        // 将结果转换为字符串
        const resultStr = typeof execResult.data === 'string'
          ? execResult.data
...
function WorkflowExecutor({
  ...
  // 处理变量值变化
  const handleVariableChange = (variableId: string, value: VariableValue['value'], entryData?: unknown) => {
    setVariableValues((prev) => ({
      ...prev,
      [variableId]: { variableId, value, entryData: entryData as Record<string, unknown> | undefined },
    }))
  }

  // 执行 SKILL 变量
  const executeSkillForVariable = async (skillId: string, params: Record<string, unknown>): Promise<SkillExecutionResult> => {
        ...
        return (
          <Select
            ...
            onChange={(val) => {
              ...
              handleVariableChange(variable.id, val, entry ? { name: entry.name, ...entry.fields } : undefined)
            }}
  ...
  const handleExecute = async () => {
    // 验证必填变量
    const missingVariables = activeVariables.filter(
      (v) => v.required && !variableValues[v.id]?.value
    )
          ...
          {activeVariables.map((variable) => (
            <div key={variable.id} className={styles.variableItem}>
              <label className={styles.variableLabel}>
                {variable.name}
                {variable.required && <Tag color="red" style={{ marginLeft: 4 }}>必填</Tag>}
              </label>
  ...
  const renderSuccessPhase = () => (
    <div className={styles.successPhase}>
      <Result
        status="success"
        title="执行成功"
        subTitle={`耗时 ${apiResult?.duration || 0}ms`}
      />
  ...
  return (
    <Modal
      title={
        <Space>
          <PlayCircleOutlined />
          {currentWorkflow ? `执行工作流: ${currentWorkflow.name}` : currentTemplate ? `执行模板: ${currentTemplate.name}` : '执行'}
        </Space>
      }
      open={open}
