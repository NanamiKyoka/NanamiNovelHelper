...
import { VOCABULARY_DEFAULT_COLORS } from '@shared/vocabulary'
  ...
  (
    ...
    const highlightText = useCallback((text: string, keyword: string): React.ReactNode => {
      ...
      return (
        <>
          {before}
          <span style={{ backgroundColor: '#ffe58f', padding: '0 2px', borderRadius: 2 }}>
            {match}
          </span>
    ...
    const columns = useMemo(() => {
      ...
      const dragHandleColumn = {
        ...
        width: 40,
        fixed: 'left' as const,
        render: () => <HolderOutlined style={{ color: 'var(--text-tertiary)', cursor: 'grab' }} />
      }
    ...
    const renderEmptyState = (): React.ReactNode => {
      ...
      if (!currentType) {
        return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="请先选择或创建词汇类型" />
      }
