...
const api = {
  ...
  window: {
    ...
    setFullScreen: (isFullscreen: boolean) =>
      ipcRenderer.send('window-set-fullscreen', isFullscreen),
  ...
  vocabulary: {
    ...
    // 设置
    getSettings: (): Promise<VocabularySettings> => ipcRenderer.invoke('vocabulary:getSettings'),
  ...
  sensitive: {
    ...
    ): Promise<number> => ipcRenderer.invoke('sensitive:importWords', words)
  },
  // 高亮配置管理
  highlight: {
  ...
  settings: {
    ...
    global: {
      ...
      getSidebarWidth: (): Promise<number> => ipcRenderer.invoke('settings:global:getSidebarWidth'),
      ...
      getApiKeyNames: (): Promise<string[]> => ipcRenderer.invoke('settings:global:getApiKeyNames'),
  ...
  backup: {
    ...
    restore: (filename: string): Promise<boolean> => ipcRenderer.invoke('backup:restore', filename),
    delete: (filename: string): Promise<boolean> => ipcRenderer.invoke('backup:delete', filename),
  ...
  relationship: {
    // 关系图管理
    getList: (): Promise<RelationshipGraphMeta[]> => ipcRenderer.invoke('relationship:getList'),
  ...
  timeline: {
    // 时间线管理
    getList: (): Promise<TimelineMeta[]> => ipcRenderer.invoke('timeline:getList'),
    ...
    ): Promise<TimelineNode | null> => ipcRenderer.invoke('timeline:addNode', timelineId, node),
    updateNode: (
      timelineId: string,
      nodeId: string,
    ...
    showImportDialog: (): Promise<string | null> => ipcRenderer.invoke('timeline:showImportDialog'),
  ...
  sequenceChart: {
    // 事序图管理
    getList: (): Promise<SequenceChartMeta[]> => ipcRenderer.invoke('sequenceChart:getList'),
  ...
  organization: {
    // 组织架构图管理
    getList: (): Promise<OrganizationGraphMeta[]> => ipcRenderer.invoke('organization:getList'),
  ...
  map: {
    ...
    update: (mapId: string, updates: import('@shared/map').UpdateMapOptions) =>
      ipcRenderer.invoke('map:update', mapId, updates),
  ...
  image: {
    ...
    delete: (imagePath: string): Promise<void> => ipcRenderer.invoke('image:delete', imagePath),
    readAsBase64: (imagePath: string): Promise<string> =>
      ipcRenderer.invoke('image:readAsBase64', imagePath),
    exists: (imagePath: string): Promise<boolean> => ipcRenderer.invoke('image:exists', imagePath),
  ...
  terminal: {
    ...
    destroy: (id: string): Promise<boolean> => ipcRenderer.invoke('terminal:destroy', id),
    list: (): Promise<TerminalInstance[]> => ipcRenderer.invoke('terminal:list'),
    getShells: (): Promise<ShellInfo[]> => ipcRenderer.invoke('terminal:getShells'),
  ...
  git: {
    // 仓库管理
    isRepo: (repoPath: string): Promise<boolean> => ipcRenderer.invoke('git:isRepo', repoPath),
    ...
    restoreStaged: (repoPath: string, filepaths: string[]) =>
      ipcRenderer.invoke('git:restore:staged', repoPath, filepaths),
    commit: (
      repoPath: string,
      options: {
        message: string
    ...
    // 分支管理
    branchList: (repoPath: string) => ipcRenderer.invoke('git:branch:list', repoPath),
    ...
    // 模式
    setMode: (mode: 'system' | 'isomorphic' | 'auto') => ipcRenderer.invoke('git:setMode', mode),
