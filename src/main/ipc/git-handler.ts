...
export function registerGitHandlers(): void {
  ipcMain.handle(GIT_CHANNELS.IS_REPO, async (_event, repoPath: string) => {
    validateParams('git:isRepo').nonEmptyString(repoPath, 'repoPath').validate()
  ...
  ipcMain.handle(GIT_CHANNELS.STATUS, async (_event, repoPath: string) => {
    validateParams('git:status').nonEmptyString(repoPath, 'repoPath').validate()
    return gitService.getStatus(repoPath)
  })

  ipcMain.handle(GIT_CHANNELS.LOG, async (_event, repoPath: string, options?: GitLogOptions) => {
    validateParams('git:log').nonEmptyString(repoPath, 'repoPath').validate()
  ...
  ipcMain.handle(
    GIT_CHANNELS.RESTORE,
    async (_event, repoPath: string, filepaths: string[], source?: string) => {
  ...
  ipcMain.handle(
    GIT_CHANNELS.RESTORE_STAGED,
    async (_event, repoPath: string, filepaths: string[]) => {
      validateParams('git:restoreStaged')
        .nonEmptyString(repoPath, 'repoPath')
        .array(filepaths, 'filepaths')
        .validate()
      return gitService.unstage(repoPath, filepaths)
    }
  )

  ipcMain.handle(
    GIT_CHANNELS.COMMIT,
    async (_event, repoPath: string, options: GitCommitOptions) => {
  ...
  ipcMain.handle(GIT_CHANNELS.BRANCH_LIST, async (_event, repoPath: string) => {
    validateParams('git:branchList').nonEmptyString(repoPath, 'repoPath').validate()
    return gitService.getBranches(repoPath)
  })

  ipcMain.handle(
    GIT_CHANNELS.BRANCH_CREATE,
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, options: GitCheckoutOptions) => {
  ...
  ipcMain.handle(
    GIT_CHANNELS.CONFIG_SET,
    async (_event, repoPath: string, key: string, value: string) => {
