...
import type { VocabularyType, VocabularyEntry, SensitiveWord } from '../types'
...
export function registerVocabularyHandlers(): void {
  ...
  ipcMain.handle('vocabulary:saveTypes', (_event, types: VocabularyType[]): void => {
    validateParams('vocabulary:saveTypes').array(types, 'types').validate()
    vocabularyService.saveVocabularyTypes(types)
  })

  ipcMain.handle(
    'vocabulary:addType',
    (_event, type: Omit<VocabularyType, 'id' | 'createdAt' | 'updatedAt'>): VocabularyType => {
  ...
  ipcMain.handle(
    'vocabulary:updateType',
    (_event, id: string, updates: Partial<VocabularyType>): VocabularyType | null => {
  ...
  ipcMain.handle('vocabulary:deleteType', (_event, id: string): boolean => {
    validateParams('vocabulary:deleteType').nonEmptyString(id, 'id').validate()
  ...
  ipcMain.handle('vocabulary:loadEntries', (_event, typeId?: string): VocabularyEntry[] => {
    if (typeId !== undefined) {
      validateParams('vocabulary:loadEntries').nonEmptyString(typeId, 'typeId').validate()
    }
    return vocabularyService.loadVocabularyEntries(typeId)
  })

  ipcMain.handle(
    'vocabulary:saveEntries',
    (_event, typeId: string, entries: VocabularyEntry[]): void => {
  ...
  ipcMain.handle(
    'vocabulary:addEntry',
    (_event, entry: Omit<VocabularyEntry, 'id' | 'createdAt' | 'updatedAt'>): VocabularyEntry => {
  ...
  ipcMain.handle(
    ...
    (_event, id: string, updates: Partial<VocabularyEntry>): VocabularyEntry | null => {
  ...
  ipcMain.handle('vocabulary:deleteEntry', (_event, id: string): boolean => {
    validateParams('vocabulary:deleteEntry').nonEmptyString(id, 'id').validate()
  ...
  ipcMain.handle('vocabulary:unlinkFile', (_event, entryId: string): boolean => {
    validateParams('vocabulary:unlinkFile').nonEmptyString(entryId, 'entryId').validate()
  ...
  ipcMain.handle('sensitive:saveWords', (_event, words: SensitiveWord[]): void => {
    validateParams('sensitive:saveWords').array(words, 'words').validate()
    vocabularyService.saveSensitiveWords(words)
  })

  ipcMain.handle(
    'sensitive:addWord',
    (_event, word: Omit<SensitiveWord, 'id' | 'createdAt' | 'updatedAt'>): SensitiveWord => {
  ...
  ipcMain.handle(
    'sensitive:updateWord',
    (_event, id: string, updates: Partial<SensitiveWord>): SensitiveWord | null => {
  ...
  ipcMain.handle('sensitive:deleteWord', (_event, id: string): boolean => {
    validateParams('sensitive:deleteWord').nonEmptyString(id, 'id').validate()
    return vocabularyService.deleteSensitiveWord(id)
  })

  ipcMain.handle(
    'sensitive:importWords',
    (_event, words: Array<Omit<SensitiveWord, 'id' | 'createdAt' | 'updatedAt'>>): number => {
      validateParams('sensitive:importWords').array(words, 'words').validate()
  ...
  ipcMain.handle(
    'vocabulary:updateSettings',
    (_event, settings: { autoCreateVocabularyFile?: boolean }) => {
      validateParams('vocabulary:updateSettings').object(settings, 'settings').validate()
